...
import { getFlatPos, meshFlatPos, toFlatPos } from './utils/vectors';
import { scene } from '../isokit.globals';
import { applyFlatPos } from '../isokit.mesh';
...
export function* onDotChange(action: any) {
  ...
  if (meshId) {
    applyFlatPos(meshId, params as IPosFlat);
  }
}
...
export function* moveDot(action: ActionDot) {
  ...
  const region = yield* select(selectors.playback.$timelineRegion);
  const { start, duration } = region;

  const localTimestamp = Math.round(duration * percent);
  const timestamp = Math.round(start + localTimestamp);
