...
import { attachGizmo } from './isokit.gizmos';
...
export const applyFlatPos = (meshId: string, flatPos?: IPosFlat) => {
  const mesh = scene.getMeshByName(meshId);
  ...
  const { x, y, z, rx, ry, rz, sx, sy, sz } = flatPos;
  ...
  if (rx !== undefined && ry !== undefined && rz !== undefined) {
    mesh.rotation.x = degreesToRadians(rx);
    mesh.rotation.y = degreesToRadians(ry);
    mesh.rotation.z = degreesToRadians(rz);
  }
